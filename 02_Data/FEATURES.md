...
> ⚠️ The "12 metadata features" table in `data_retrieval.md` §5.4 includes view/like/comment counts as features #1–3. They are **label-only** (see `02_Data/README.md`). The actual model-facing metadata list is the rows marked "model input" above — 12 rows mapping to 13 `meta__` columns (the channel-maturity row holds three columns; `upload_date` is consumed by the `sched__` group) — plus the §6 schedule features once backfilled.
