...
| `is_short` | **filter** (model input allowed) | **Definitive** YouTube Shorts verdict — not a duration proxy. From the URL test `https://www.youtube.com/shorts/<id>`: 200 with a playable page = Short, 303 → `/watch?v=` = regular video (`02_Data/yt_shorts.py`; two EU/edge pitfalls handled, verified 2026-08-29: without a consent cookie every request 302s to a consent page, and a **deleted/private video also answers 200** — its page's `playabilityStatus: ERROR` marks it unknown rather than Short). The Data API has no Shorts flag and duration is only a proxy (Shorts can be ≤180s since Oct 2024; a 2-min horizontal video is not a Short). Retrospective: `backfill_published_at.py --shorts-only` (a full run never re-probes or overwrites a stored verdict); tracker: verified at admission (cap-bounded) and via `--check-shorts` for older rows, with blank verdicts healed a few hundred per scheduled tick. `""`/NaN = unknown (consent page, deleted/private, rate-limited) — never a verdict | `metadata_extra.json`, `cohort.csv` |
