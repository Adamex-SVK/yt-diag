...
- **Concurrency exposed a real DLL race (found + fixed 2026-08-22):** with `--workers 3` on the school computer, `[WinError 1114] DLL-Initialisierungsroutine ist fehlgeschlagen` on `torch\lib\c10.dll` recurred in ~15% of videos — not the rare one-off it looked like single-threaded. Root cause: `import whisper` (which triggers torch's native DLL loading) sat *outside* `_whisper_lock`, so multiple threads could hit their first import simultaneously and race on Windows' DLL loader. Fixed by moving the import inside the lock, serializing the entire first-import-and-load sequence. Could only be diagnosed by actually reproducing it at scale on the real machine — the earlier single 1/20 failure looked environmental (antivirus, memory pressure) until concurrency made the real cause obvious.
