...
recovers `is_short` for 99.0% of videos, and raw thumbnail brightness predicts format at
direction-free AUC 0.925. Any vision result must be read against a format-only baseline, or it may
be measuring aspect ratio and thumbnail composition rather than content quality.
...
- Raw thumbnail brightness predicts format at direction-free **AUC 0.925**; a heuristic content
  crop lowers that to **0.559**. The heuristic also crops many regular thumbnails and is not a
  verified pillarbox label, but it confirms that the raw whole-thumbnail statistic is dominated
  by format-specific composition/padding.
...
explicit note that the engineered visual features contain format-specific composition cues.
...
- **Historical CCT defect — fixed by v3 after this label EDA.** The collected formula produced
  values to 5.0 × 10⁹ K and 27 negative temperatures. All 39,060 stored images were subsequently
  recomputed with linear-light sRGB→XYZ, nearest-Planckian-locus projection, signed Duv gating and
  endpoint rejection. Current coverage is 1,854/1,860 thumbnails and 36,652/37,200 frames; current
  ranges and descriptive summaries are generated by `eda_features.py`. Do not reuse the old
  83-row flag count or the old CCT-dependent plots/baselines.
