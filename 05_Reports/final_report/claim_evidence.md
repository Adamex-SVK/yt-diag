...
| Test-set metrics | registered, matures post-submission | pre-registered in `03_Models/final_model_policy.json` (frozen 2026-09-03): eligible prospective videos admitted from 2026-09-04 need a 30-day outcome, so the first external metric is computable 2026-10-04 — after submission by design. The paper states this explicitly (Experiment: "reserved for the frozen external evaluation and contributes no result in this paper") and reports repeated validation, never a test claim | Experiment, Limitations |
...
| Correlation-reduced audio (49 eGeMAPS representatives, |ρ|>0.75 clustering, + 4 pause features = 53 cols) reproduces the lift within noise: 0.633 ± 0.030 — column selection used whole-dataset correlations/gain rather than being redone per outer fold, so is optimistic relative to the unaffected 88-column result; caveated in main.tex (2026-09-07) | confirmed | same run; `results/audio_ablation.json → reduced_audio` | Experiments |
...
| Cluster bootstrap by channel (not by individual video — videos cluster by channel, the same reason splits are channel-grouped; resampled within each seed's validation fold, hyperparameters held fixed at their already-tuned values, no re-search): audio's lift is +0.018 AUC, 95% CI [-0.011, +0.046] — interval spans zero, reported as a descriptive tail probability, not a null-centered hypothesis-test p-value | confirmed (revised 2026-09-07: the original version of this script resampled individual videos rather than channels, which understates sampling variance since videos are not exchangeable within a channel; also dropped the "p-value" framing since the bootstrap distribution is centered on the point estimate, not on zero) | `run_ablation_significance.py`; `04_Experiments/runs/ablation_significance/results.json → audio_isolated_vs_reference` | Experiments, Limitations |
| Same test on the visual-engineered drop: -0.023 AUC, channel-level 95% CI [-0.045, -0.002] — entirely below zero, a consistent drop, unlike audio's inconclusive lift | confirmed (revised 2026-09-07, same channel-bootstrap fix as above) | `run_ablation_significance.py`; `results.json → visual_engineered_vs_reference` | Experiments |
| Post-freeze audio+text fusion: engineered audio + frozen ModernBERT text embeddings, combined in the same linear/MLP fusion architecture as the frozen finalist. Text+meta 0.581/0.578 (linear/MLP), audio+meta in this architecture 0.608/0.585 (below audio's 0.632 under tuned XGBoost), combined 0.578/0.597. Architecture-dependent: linear falls (1/5 paired wins vs. audio alone), MLP rises modestly (4/5 paired wins) — text is not uniformly harmful. Neither head beats tuned metadata+schedule (0.619) | confirmed (revised 2026-09-07: the original wording said "text adds nothing" and "below audio alone" for both heads, which was false for the MLP head — it rises from 0.585 to 0.597 and wins 4/5 paired seeds; also dropped the unsupported causal claim that audio's edge is "tied to tree-based modelling") | `run_audio_text_fusion.py --seeds 0,1,2,3,4`; `publish_audio_text_fusion.py`; `results/audio_text_fusion.json` | Experiments |
| Conclusion and Limitations now state the audio-isolation lift's bootstrap interval spans zero (previously stated flatly as an improvement, then as "not statistically significant" against a p-value that was never a real null-centered test — both stages superseded by the 2026-09-07 channel-bootstrap revision) | confirmed | `main.tex` Conclusion + Limitations, matches the significance test above | Conclusion, Limitations |
| Metadata+schedule XGBoost reference value differs between `results/tuned_baselines.json` (0.619±0.022) and `results/audio_ablation.json`'s own recomputed reference (0.614±0.022) for a nominally identical protocol/feature set — `requirements-ml.txt` pins neither `scikit-learn` nor `xgboost` exactly, and the two runs are two days apart, so dependency or split-construction drift is the likely cause. Not re-run in this session (no controlled environment diff available); disclosed as a limitation instead. Each post-freeze comparison stays internally paired (same run, same environment), so within-run deltas are trustworthy even though the two absolute references are not interchangeable | limitation | `results/tuned_baselines.json` vs `results/audio_ablation.json`; `requirements-ml.txt` (unpinned versions) | Limitations |
| Liu et al. bibliography entry author names corrected to match the publisher (Yanchao Liu, Qinling Lei, Zhen Liu, MingChao Zhao, Ze Xu, Lei Shi) | confirmed | `references.bib` (`liu2026survey`), fixed 2026-09-07 | References |
...
- [x] Every `pending` row above is either resolved or removed — the last one (`Test-set metrics`) resolved 2026-09-07 by pre-registration: the frozen policy's own eligibility (admission ≥ 2026-09-04 + 30-day outcome) makes it computable no earlier than 2026-10-04, and the paper already frames it as contributing no result here
