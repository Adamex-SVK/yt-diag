...
| **Test-set metrics** | **pending, post-submission** | evaluate the frozen selected model ONCE after the modelling policy is final; the frozen prospective cohort begins 2026-09-04, so its earliest 30-day-outcome maturity is ~2026-10-04 -- after any near-term submission deadline. Not a submission blocker; main.tex §"Evaluation protocol" (line 305) already frames this correctly as reserved for after submission. | Experiments |
...
- [ ] Every `pending` row above is either resolved or removed from the report — only `Test-set metrics` remains, and it is a post-submission evaluation (earliest 30-day maturity ~2026-10-04), not a submission blocker; nothing here should block final proofing or the `\draftfalse` flip
