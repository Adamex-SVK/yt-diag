...
**Split**: channel-grouped, label-stratified 60/20/20 via `StratifiedGroupKFold` — no channel spans splits (asserted). The original fixed baselines use validation for their displayed F1 threshold. The tuned workflow selects both hyperparameters and threshold on inner, channel-grouped training folds, leaving outer validation for evaluation. The **test split remains sealed until the final policy is frozen**.
...
| `tests/test_tuning.py` | Inner-fold channel isolation, coverage, reproducibility, threshold provenance, and sealed-test checks | Passing |
...
| `ytdiag/visual_ablation.py` | Revision-pinned DINOv2-S/B, CLIP ViT-B/32 and ResNet-50 thumbnail encoders; aspect/pooling controls and mean frame aggregation | Done |
...
- **Controlled visual ablation**: DINOv2-base underperforms DINOv2-small on thumbnails (linear AUC 0.551 versus 0.563), CLIP ViT-B/32 is slightly higher at 0.570, and ResNet-50 is near chance at 0.520. Mean DINOv2-small features over all 20 stored frames reach MLP AUC 0.589 ± 0.023. Combining thumbnail, frames, field-aware text, metadata and schedule reaches 0.609 ± 0.026, versus 0.619 ± 0.022 for tuned metadata+schedule XGBoost; test remains untouched.
